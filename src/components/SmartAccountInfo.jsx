...
const SmartAccountInfo = () => {
  ...
  return (
    <div className="bg-gray-800/50 rounded-lg p-4 mb-4 border border-gray-700">
      <h3 className="text-lg font-semibold text-white mb-2">{t('smart_account_info.title')}</h3>
      <div className="space-y-2 text-sm">
        <div className="flex justify-between">
          <span className="text-gray-400">{t('smart_account_info.address')}:</span>
          <span className="text-white font-mono">{address?.slice(0, 6)}...{address?.slice(-4)}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-400">{t('smart_account_info.type')}:</span>
        ...
        {isSmartAccount && (
          <div className="mt-2 p-2 bg-blue-900/30 rounded border border-blue-700">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-blue-300 text-xs">{t('smart_account_info.smart_features_active')}</span>
            </div>
          </div>
        )}
      </div>
