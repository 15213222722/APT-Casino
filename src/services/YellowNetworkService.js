...
class YellowNetworkService {
  ...
  async initialize() {
    ...
    this.initializingPromise = (async () => {
      try {
        ...
        // Create One Chain Network client for on-chain operations
        const arbitrumClient = createPublicClient({
          chain: arbitrumSepolia,
          transport: http(process.env.NEXT_PUBLIC_ARBITRUM_SEPOLIA_RPC || 'https://sepolia-rollup.arbitrum.io/rpc')
        });

        console.log('🔗 Primary Network: One Chain Network (Chain ID: 421614)');
  ...
  async connect(channelId = null, accessToken = null) {
    ...
    this.connectingPromise = (async () => {
      ...
      console.log('🔗 State channels active on One Chain Network settlement layer');
