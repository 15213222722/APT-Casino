...
async function checkTreasuryContract() {
  ...
  // One Chain Testnet RPC
  const provider = new ethers.JsonRpcProvider('https://testnet-rpc.onechain.xyz');
  ...
  try {
    ...
    if (code !== '0x') {
      ...
      console.log('💰 Treasury Balance:', ethers.formatEther(balance), 'OCT');
    } else {
    ...
    console.log('💰 Target Balance:', ethers.formatEther(targetBalance), 'OCT');
